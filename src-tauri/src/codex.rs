...
fn find_window<'a>(
    ...
    let duration_matches = |window: &UsageWindow| {
    ...
    for name in names {
        if let Some(value) = rate_limit.get(*name) {
            if let Some(window) = parse_window(Some(value)) {
                if window.window_seconds == 0 || duration_matches(&window) {
                    return Some(value);
                }
            }
        }
    ...
    if let Some(map) = rate_limit.as_object() {
    ...
    for key in [
        ...
        for item in items {
            ...
            let matches_duration = window.window_seconds > 0 && duration_matches(&window);
...
pub async fn fetch_snapshot(client: &reqwest::Client) -> ProviderSnapshot {
    let auth = match load_auth() {
        Ok(value) => value,
        Err(message) => {
            return ProviderSnapshot::failure_for("codex", "CODEX", "signed_out", message)
        }
    };
    ...
    let usage_response = match usage_result {
        ...
        Ok(response) => {
            ...
            return ProviderSnapshot::failure_for("codex", "CODEX", status, message);
        }
        Err(_) => {
    ...
    if short_window.is_none() && weekly_window.is_none() {
        return ProviderSnapshot::failure_for(
            ...
            "CODEX",
            "unavailable",
            "Quota response does not include a supported usage window.",
        );
    }
...
mod tests {
    ...
    #[test]
    fn classifies_generic_primary_window_by_actual_duration() {
        let rate_limit = serde_json::json!({
            "primary_window": {
                "used_percent": 5,
        ...
        assert!(find_window(
            &rate_limit,
            &["primary_window", "short_window", "five_hour_window"],
            18_000
        )
        .is_none());
        let weekly = parse_window(find_window(
