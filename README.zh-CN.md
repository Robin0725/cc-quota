...
## CC Quota 0.5.14 更新亮点

- 修复取消显示悬浮窗时应用崩溃、顶部胶囊随之消失的问题；原生菜单会等当前 macOS 菜单事件结束后再刷新。
- 左键点击菜单栏胶囊即可在原有「5 小时额度」与「周额度」之间切换，无需打开悬浮窗。
- 周额度沿用服务商身份色填充，并以 `5d`、`1d8h`、`18h` 等紧凑格式显示重置倒计时。
- 周额度降至 20% 或以下时，每 2% 出现一道上下交替渐隐的精细刻度。
- 10% 节点使用一根完整的白色竖线；5 小时胶囊设计保持不变，右键仍打开完整菜单。

## CC Quota 0.5.12 更新亮点

- 焦点切换到另一家服务商时立即强制刷新额度；Kimi CLI 刚唤醒并续签短期令牌后，悬浮窗无需展开或等待五分钟即可切换。
- Kimi 已保持聚焦但额度快照仍不可用时，会按上限退避继续恢复，覆盖“焦点未变化、凭证随后续签”的情况。
- 周额度动态条始终保留各服务商自己的身份色，不再让 Claude 或 Kimi 被同一种警告色覆盖。
- 每个精确构建第一次在未授权状态启动时，会自动修复 CC Quota 自身失效的 ad-hoc 辅助功能记录；最终授权仍必须由用户在 macOS 中亲自批准。
- 启动时补录已有的 Codex、Claude 与 Kimi 提示活动，重启应用后不会暂时忘记最近使用的是哪个终端。
- 精确恢复详情卡原来的 `3px` 左边框装饰条，只让这条原生边框从底部向上展示周额度；不再使用圆环蒙版或额外外框。
- 悬浮球移植同样的左边框条，同时与键盘焦点外框彻底分开；悬停仍可读取精确周额度。
- 只要周额度存在就始终显示这条边缘；即使主数字本身已经回退到周额度，视觉总量刻度也不会消失。

## CC Quota 0.5.7 更新亮点

- 把详情卡左侧原来的装饰色条升级为周额度动态条：从底部向上填充，长度就是当前周额度剩余比例。
- 收起的悬浮球也加入同款周额度条；鼠标悬停时，底部圆点会暂时切换成精确的周额度数字。
- 当主数字本身已经回退为周额度时，不重复显示动态条；缺失周额度时也不伪造第二项数据。

## CC Quota 0.5.6 更新亮点

- 把每个窗口的重置倒计时放到它自己的数字旁边。此前详情卡只有一个倒计时，默默归属于驱动大数字的那个窗口，于是周额度旁边摆着的是一个并不属于它的时间。
- 周额度有了自己的倒计时；而当某个服务商只有周额度这一个窗口时，下方那行整个略去，不再把同一个百分比在一张卡上印两遍。
- 详情卡拆成「数字栏」与「周额度栏」两列，以左侧色条区分，数字和它的倒计时读起来是一叠，而不是交错的几行。
...
- 左键点击菜单栏胶囊可在现有的 5 小时视图与周额度视图之间切换；右键点击打开完整的 CC 菜单。
- 5 小时胶囊保持原有视觉不变。周额度继续沿用服务商彩色填充，中央改为紧凑的重置倒计时；仅在剩余 20% 及以下显示每格 2% 的上下交替渐隐刻度，10% 边界是一根完整白线。
...
- 跟随你最先点击的那个窗口：当前台应用或其窗口标题指向某个助手时（终端会用正在运行的命令来命名窗口），悬浮窗会立即切过去。读取窗口标题需要 macOS 辅助功能权限；每个精确的 ad-hoc 构建第一次在未授权状态启动时，应用会自动只清理自身的旧记录，并触发一次系统授权流程。托盘菜单仍可手动重试，用户也仍需亲自打开 macOS 权限开关；标题只在内存中匹配，绝不存储或记录。
...
时间圆点只在 5 小时视图下出现。左键点击状态项即可在不打开窗口的情况下查看周额度填充与紧凑重置倒计时，再点一次回到 5 小时视图。周额度剩余 20% 及以下时，每个轻刻度代表 2%，第 5 条完整白线标记 10%。右键点击打开完整菜单。

打开菜单可以查看重置时间、立即刷新、显示或隐藏悬浮窗、切换置顶、解除鼠标穿透、切换语言、控制开机自启，或退出 CC。

## 悬浮窗

悬浮窗是可选的，默认关闭。启用后，它会为你最近使用的那个助手显示一个主导百分比；只要周额度存在，左侧圆角边缘就同步显示周额度，悬停可读精确数字。点击它会在正下方打开详情面板，每个已登录服务商一张卡片，附有精确的重置时间，以及与 5 小时数据并列的周额度数字。
