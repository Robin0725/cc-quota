...
In memory only, and never written to disk, CC also holds the time of the most recent prompt-history (or session-directory) change per provider, which is how it decides whose quota to show.
