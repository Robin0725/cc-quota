...
import { clampPercent, displayableSnapshots, formatResetDate, formatResetTime, isSnapshotDisplayable, needsFastRefresh, preferredWindow, quotaTier } from "./format";
...
describe("quota formatting", () => {
  ...
  it("prefers a 5-hour window even at 0%, and falls back only when it is absent", () => {
    const base: Omit<ProviderSnapshot, "shortWindow" | "weeklyWindow"> = {
      provider: "claude",
      displayName: "CLAUDE",
      plan: "MAX",
      resetCredits: null,
    ...
    const weeklyWindow = { remainingPercent: 83, resetsAt: "2026-07-10T00:00:00Z", windowSeconds: 604_800 };
    const shortWindow = { remainingPercent: 0, resetsAt: "2026-07-07T02:00:00Z", windowSeconds: 18_000 };
    expect(preferredWindow({ ...base, shortWindow, weeklyWindow })).toEqual({ kind: "short", value: shortWindow });
  ...
  it("keeps stale data displayable for a day but expires it after 24 hours", () => {
    ...
    const snapshot: ProviderSnapshot = {
      ...
      displayName: "CODEX",
      plan: "PRO",
      shortWindow: { remainingPercent: 50, resetsAt: null, windowSeconds: 18_000 },
      weeklyWindow: null,
      resetCredits: null,
      resetCreditExpiresAt: [],
  ...
  it("selects only providers with a real displayable window", () => {
    ...
    expect(displayableSnapshots([codex, claude])).toEqual([codex]);
  });
